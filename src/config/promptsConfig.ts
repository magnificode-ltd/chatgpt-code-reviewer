...
const promptsConfig: { [key in Prompt]: string } = {
  ...
  [Prompt.PREPARE_SUGGESTIONS]:
    'There is an array. Every object of this array has a `patch` and `filename` properties. You should make the next steps: 1. Based on `patch` property, provide a list of suggestions how to improve the code with examples according to coding standards and best practices. 2. All suggestions should be separated by `filename`',
};
